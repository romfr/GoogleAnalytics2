 ...
 * @package    Mage_GoogleAnalytics
 */
class Rom_GoogleAnalytics2_Model_Observer extends Mage_GoogleAnalytics_Model_Observer
{
    ...
    public function setGoogleAnalyticsOnOrderSuccessPageView(Varien_Event_Observer $observer)
    {
        ...
        $block = Mage::app()->getFrontController()->getAction()->getLayout()->getBlock('romgoogle_analytics2');
        if ($block) {
    ...
    public function injectAnalyticsInGoogleCheckoutLink(Varien_Event_Observer $observer)
    {
        ...
        if (!$block || !Mage::helper('romgoogleanalytics2')->isGoogleAnalyticsAvailable()) {
            return;
        }
